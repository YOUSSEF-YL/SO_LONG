...
/*   Updated: 2023/01/03 13:39:28 by ybachar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_hook(int keycode,t_vars *vars)
{
   t_intvars intvar;
    ...
    if ((keycode == 126 || keycode == 13)  && vars->map[intvar.j-1][intvar.i] != '1') //up
		c = move_to(vars,intvar.j-1,intvar.i,intvar,c);
    if ((keycode == 123 || keycode == 0)  && vars->map[intvar.j][intvar.i-1] != '1') //L
		c = move_to(vars,intvar.j,intvar.i-1,intvar,c);
    if ((keycode == 125 || keycode == 1)  && vars->map[intvar.j+1][intvar.i] != '1') //d
		c = move_to(vars,intvar.j+1,intvar.i,intvar,c);
    if ((keycode == 124 || keycode == 2)  && vars->map[intvar.j][intvar.i +1] != '1') //R
		c = move_to(vars,intvar.j,intvar.i+1,intvar,c);
	if (keycode == 53)
		exit(0);
    return (0);
